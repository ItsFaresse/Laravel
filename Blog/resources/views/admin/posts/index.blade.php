...
@section('content')
	...
	<ul class="nav navbar-nav">
        <li><a href="{{ route('posts.index') }}">Tous les articles</a></li>
        <li><a href="{{ route('posts.create') }}">Créé un article</a>
    </ul>
                                ...
                                <tr class="@if($post->is_published) warning @endif">
                                    ...
                                    <td style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                        {!! Form::open(array('route' => array('posts.destroy', $post->id), 'method' => 'POST', 'id' => 'postForm' )) !!}
                                            <a class="btn btn-small btn-success" href="{{ URL::to('admin/posts/' . $post->id) }}" style="margin: 5px;">Voir l'article</a>
                                            <a class="btn btn-small btn-info" href="{{ URL::to('admin/posts/' . $post->id . '/edit') }}" style="margin: 5px;">Modifier l'article</a>
                                            <input name="_method" type="hidden" value="DELETE">
                                            <input name="_token" value="{{ csrf_token() }}" type="hidden">
                                            <input class="btn btn-small btn-warning" value="Supprimer l'article" style="margin: 5px;" type="submit">
                                        {!! Form::close() !!}
                                    </td>
                                </tr>
                            @endforeach
